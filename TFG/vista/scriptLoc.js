document.addEventListener('DOMContentLoaded', () => {
  ...
  const containerwr = document.querySelector('.content-wrapper');
  ...
  const loadItems = () => {
    fetch(`https://api.api-onepiece.com/v2/locates/en?offset=${offset}&limit=${limit}`)
      ...
      .then(data => {
        ...
        characters.slice(offset, offset + limit).forEach(character => {
          ...
          container.insertBefore(box, space, loadMoreButton);
